"""Phase 2 live verification: run Report 3 or 4 and capture phase2 logs."""
...
class Phase2LogCapture(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        if not (event and str(event).startswith("phase2_")) and "phase2_" not in msg:
            return
        entry = {
            "message": msg,
            "event": event,
            ...
            "section_name": getattr(record, "section_name", None),
...
async def run_slug(slug: str) -> dict:
    ...
    phase2_events = [
        ...
        if (e.get("event") or "").startswith("phase2_")
        or "phase2_" in (e.get("message") or "")
    ]
    ...
    dest = ROOT / "storage" / "debug" / f"phase2_live_{slug}.json"
...
async def main() -> int:
    slug = sys.argv[1] if len(sys.argv) > 1 else "train-no"
