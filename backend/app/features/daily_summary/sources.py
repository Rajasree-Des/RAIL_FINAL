...
class ReportSource:
    ...
    bottom_report: BottomReportResult | None = None
...
def _path_includes_run_id(path: Path, run_id: str) -> bool:
    if not run_id:
        return True
    return run_id in path.parts


def _validate_run_scoped_path(
    path: Path,
    *,
    slug: str,
    run_id: str,
    ...
    """Prefer run_id in artifact paths; accept legacy slug-only paths from result_json."""
    if not run_id or _path_includes_run_id(path, run_id):
        return True
    sources.validation_notes.append(
        f"{slug}: artifact path does not include run_id={run_id} "
        f"(accepted from run result_json): {path}"
    )
    return True
...
def resolve_run_sources(run: AutomationRunModel) -> RunSources:
    ...
    for slug in SUMMARY_SOURCE_SLUGS:
        ...
        if report.status not in TERMINAL_REPORT_STATUSES:
            ...
            sources.missing_reports.append(slug)
            continue
        if report.status in {"failed", "skipped"} and not report.source_csv_path and not report.source_paths:
            sources.missing_reports.append(slug)
...
def _load_report_data(report: ReportSource, sources: RunSources) -> None:
    slug = report.slug
    if slug == "bottom-report":
        _load_bottom_report(report, sources)
        return
    if slug == "types":
    ...
    if not _validate_run_scoped_path(
        path, slug=slug, run_id=sources.run_id, sources=sources
    ):
        report.validation_error = "artifact path does not belong to this run"
        sources.missing_reports.append(slug)
        return
...
def _load_bottom_report(report: ReportSource, sources: RunSources) -> None:
    path = _safe_path(report.source_csv_path)
    if path is None and report.source_paths:
        path = _safe_path(report.source_paths[0])
    if path is None:
        report.validation_error = "bottom-report result.json missing"
        sources.missing_reports.append("bottom-report")
        sources.validation_notes.append(f"bottom-report: {report.validation_error}")
        return

    if not _validate_run_scoped_path(
        path, slug="bottom-report", run_id=sources.run_id, sources=sources
    ):
        report.validation_error = "artifact path does not belong to this run"
        sources.missing_reports.append("bottom-report")
        return
    ...
    try:
        bottom_result = BottomReportResult.load(path)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        report.validation_error = f"failed to read result.json: {exc}"
        sources.missing_reports.append("bottom-report")
        sources.validation_notes.append(f"bottom-report: {report.validation_error}")
        return

    report.bottom_report = bottom_result
    report.available = True
    report.source_csv_path = str(path)
...
def _load_types(report: ReportSource, sources: RunSources) -> None:
    ...
    if not _validate_run_scoped_path(
        index_path, slug="types", run_id=run_id, sources=sources
    ):
        report.validation_error = "artifact path does not belong to this run"
        sources.missing_reports.append("types")
        return
